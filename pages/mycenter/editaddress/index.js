...
Page({
  data: {
    ...
    showPage:0,
    showTitBar:false,
    changeDefaultAddressState:false,
  ...
  changeDefaultAddress:function(_t){
    if(this.data.changeDefaultAddressState){
      return;
    }else{
      this.setData({
        changeDefaultAddressState:true
      })
    }
    ...
    if(cur_data){
      ...
      let reqData = {
        ...
        is_default:(cur_data.selected?1:0),
  ...
  deleteAddress:function(ev){
    if(this.data.deleteAddressState){
      return;
    }else{
      this.setData({
        deleteAddressState:true
      })
    }
